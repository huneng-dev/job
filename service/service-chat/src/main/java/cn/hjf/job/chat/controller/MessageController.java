...
public class MessageController {
    ...
    @PostMapping("/sendGetResumeMessage")
    public Result<Message> sendGetResumeMessage(Principal principal, @RequestBody @Valid Message message) {
        try {
            if (!message.getMessageType().equals(MessageType.GET_RESUME)) {
                return Result.fail();
            }
     ...
     * @return 返回发送的消息对象，如果发送失败则返回失败结果
     */
    @PreAuthorize("hasAnyRole('ROLE_USER_CANDIDATE')")
    @PostMapping("/sendResumeDeliveryMessage")
    public Result<Message> sendResumeDeliveryMessage(Principal principal, @RequestBody @Valid Message message) {
        try {
            if (!message.getMessageType().equals(MessageType.SEND_RESUME)) {
                return Result.fail();
            }
